...
CLASS lcl_flight DEFINITION ABSTRACT.
  ...
  PUBLIC SECTION.
    INTERFACES lif_output.      "Ex. 21
    ALIASES get_output FOR lif_output~get_output.

    TYPES tap TYPE STANDARD TABLE OF REF TO lcl_flight WITH DEFAULT KEY. "Ex. 20: collection-type
  ...
  PUBLIC SECTION.
    ...
    METHODS find_passenger_flight
      IMPORTING
        ...
        i_seats           TYPE i
      EXPORTING
        e_flight          TYPE REF TO lcl_flight "Ex. 20 Up cast -> Passenger flight
        e_days_later      TYPE i.
    ...
    METHODS find_cargo_flight
      ...
      EXPORTING
        e_flight          TYPE REF TO lcl_flight  "Ex. 20 Up cast
        e_days_later      TYPE i.
...
*    DATA cargo_flights TYPE lcl_cargo_flight=>tt_flights.
    ...
    DATA flights TYPE lcl_flight=>tap.   "Ex. 20 Collection füür alle Flüge
    DATA pf_count TYPE i.               "Ex 20 Anzahl der Passengierflüge
    DATA cf_count TYPE i.               "Ex 20 Anzahl der Frachtflüge
...
*    name = carrier_id && ` ` && name.    "Delete Übung 11
    ...
    DATA(cargo_flights) =                                    "Übung 20
        lcl_cargo_flight=>get_flights_by_carrier(
              i_carrier_id    = i_carrier_id ).
    cf_count = lines( cargo_flights  ).                 "Ex. 20
