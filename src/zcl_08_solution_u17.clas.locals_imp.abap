...
CLASS lcl_passenger_flight DEFINITION .
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA flights_buffer TYPE HASHED TABLE OF st_flights_buffer
                              ...
                              WITH NON-UNIQUE SORTED KEY sk_carrier_id COMPONENTS carrier_id.
    ...
    CLASS-DATA connections_buffer TYPE HASHED TABLE OF st_connections_buffer      "EX 16
...
*    ENDLOOP.
  ...
  METHOD get_flights_by_carrier.
    ...
    IF NOT line_exists( flights_buffer[ key sk_carrier_id carrier_id = i_carrier_id ] ).       "Es. 17          "Ex. 14
...
*          ORDER BY flight_date   "Übung 13
     APPENDING CORRESPONDING FIELDS OF TABLE @flights_buffer.                                           "Ex. 14
*      SORT flights_buffer BY carrier_id connection_id flight_date.                                      "Ex. 14.4
*    DELETE ADJACENT DUPLICATES FROM flights_buffer COMPARING carrier_id connection_id flight_date.     "Ex. 14.4
    ENDIF.                                                                                              "Ex. 14
...
*    ENDLOOP.
    ...
    "Ex. 14
    r_result = VALUE #( FOR <line> IN flights_buffer                            "Ex. 15 Inline filed symboles <line>
                           using kEY sk_carrier_id                               "Ex. 17
                           WHERE ( carrier_id = i_carrier_id )
                            (
