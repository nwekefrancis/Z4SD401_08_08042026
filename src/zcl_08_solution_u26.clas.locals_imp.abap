...
"! Jede Instanz wird durch folgende Attribute eindeutig idenfiziert:<br/>
"! {@link .lcl_flight.data:carrier_id},<br/>
"! {@link .lcl_flight.data:connection_id},<br/>
"! {@link .lcl_flight.data:fligth_date}.
CLASS lcl_flight DEFINITION ABSTRACT.  "*********"Abstract CLASS 'lcl-flight' is defined here

  PUBLIC SECTION.
    INTERFACES lif_output. "Übung 21
    ALIASES get_output FOR lif_output~get_output.     "***************here a METHOD 'get_out' is called here****************************************
    ...
    METHODS: get_connection_details   "*************************************"Here a METHOD 'get_connection_details' is declared*************************
      RETURNING
        VALUE(r_result) TYPE st_connection_details.

    METHODS constructor               "*************************************"Here a CONSTRUCTOR METHOD 'constructor' is declared************************
      IMPORTING
  ...
  PROTECTED SECTION.
    ...
    METHODS get_description RETURNING VALUE(r_result) TYPE string_table.  "*****"Here a METHOD 'get_description' is declared****************************
...
CLASS lcl_flight IMPLEMENTATION.
  ...
  METHOD constructor.    "+++++++++++Implementing the the 'constructor' method+++++++++++++++++
  ...
  METHOD get_connection_details.    "+++++++++++Implementing the 'get_connection_details' method+++++++++++++++++
    r_result = me->connection_details.
  ENDMETHOD.

  METHOD get_description.           "+++++++++++Implementing the 'get_description' method++++++++++++++++++++++++
...
*
  ...
  METHOD lif_output~get_output.      "Übung 21      "+++++++++++Implementing the 'get_output' method++++++++++++++++++++++++
...
"! Flight Carrier<br/>
"! Eine factory Logik stellt sicher, dass nur eine Instanz  proo CAARID erstellt wird.
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! Factory Method gibt eine Instanz der Klasse zurück
    "! @parameter i_carrier_id | Drestellige Identifikation des CARRID
    "! @parameter r_result | Referenz auf die Instanz
    "! @raising zcx_08_failed | Ausnahmertext bei Fehlgeschlagene Instanzierung
    CLASS-METHODS get_instance                            "Übung 23
      IMPORTING i_carrier_id    TYPE /dmo/carrier_id
      RETURNING VALUE(r_result) TYPE REF TO lcl_carrier
      RAISING   zcx_08_failed.
    ...
    "! Suche nach einem <strong>Passagierflug</strong> zwischen zwei Flughäfen
    "! <ul>
    "!  <li>Der Flug liegt am oder nach dem Datum</li>
    "!  <li>Hat eine Mindeszahl verfügbare Plätze frei</li>
    "! </ul>
    "! @parameter i_airport_from_id |<em>Abflughafen</em>
    "! @parameter i_airport_to_id |<em>Zielflughaafen</em>
    "! @parameter i_from_date | Erstes mögliches Flugdatum
    "! @parameter i_seats |Mindeszahl der verfugbaren Plätze
    "! @parameter e_flight | Gefundener Flug
    "! @parameter e_days_later | Anzahl der Tage nach dem Wunschdatumg
...
*    DATA name TYPE string.
    ...
    AUTHORITY-CHECK OBJECT '/LRN/CARR'
                 ...
                 ID 'ACTVT' FIELD '03'.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_08_failed
        EXPORTING
          textid     = zcx_08_failed=>carrier_no_read_auth
*          previous   =
          carrier_id = i_carrier_id.
...
*     FIELDS concat_with_space( carrier_id, name, 1 ) as name, currency_code    "Übung 11 mit as name
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_08_failed
        EXPORTING
          textid     = zcx_08_failed=>carrier_not_exist
*          previous   =
          carrier_id = i_carrier_id.
