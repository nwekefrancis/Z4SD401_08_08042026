...
CLASS lcl_passenger_flight DEFINITION .
  ...
  PRIVATE SECTION.
    ...
    TYPES : BEGIN OF st_connections_buffer,
              ...
              timzone_from    TYPE /lrn/airport-timzone,             "Ex. 10. This line is added
              timzone_to      TYPE /lrn/airport-timzone,             "Ex. 10  This line is added
              duration        TYPE i,

              "First step in solving for Ex 7
...
*    "Ex. 7
*    SELECT FROM /lrn/airport FIELDS airport_id, timzone INTO TABLE @DATA(airpots).
    ...
    SELECT FROM /dmo/connection
      ...
      LEFT OUTER JOIN /lrn/airport AS b
        ...
        FIELDS carrier_id, connection_id, airport_from_id, airport_to_id, departure_time, arrival_time,
          a~timzone AS timzone_from, b~timzone AS timzobe_b
            INTO TABLE @connections_buffer.
    ...
    LOOP AT connections_buffer INTO DATA(connection).
      "Local Zeit wird umgerechnet auf UTC Zeit d.h. wenn Frankfurt 11:00 ist: UTC -> 10:00 Uhr
      CONVERT DATE today TIME connection-departure_time
          TIME ZONE connection-timzone_from                 "Ex. 10
              INTO UTCLONG DATA(departure_utclong).

      "Local Zeit wird umgerechnet auf UTC Zeit. dann wird New York auf 12:00 Uhr: UTC -> 16:00 Uhr
      CONVERT DATE today TIME connection-arrival_time
         TIME ZONE connection-timzone_to                    "Ex. 10
             INTO UTCLONG DATA(arrival_utclong).

      connection-duration = utclong_diff( high = arrival_utclong low = departure_utclong ) / 60.

      MODIFY connections_buffer FROM connection TRANSPORTING duration.  "End of Ex. 7 *******
  ...
  METHOD get_flights_by_carrier.
    ...
    SELECT FROM /lrn/passflight
        FIELDS carrier_id, connection_id, flight_date, price, currency_code,            "Ex. 11
            plane_type_id, seats_max, seats_occupied,                                   "Ex. 11
            seats_max - seats_occupied AS seats_free                                    "Ex. 11
  ...
  METHOD constructor.
    ...
    TRY.
      ...
      CATCH cx_root.
        SELECT SINGLE
          FROM /lrn/passflight
        FIELDS carrier_id, connection_id, flight_date, plane_type_id, seats_max, seats_occupied, price, currency_code,      "Ex. 11
            seats_max - seats_occupied AS seats_free                                                                        "Ex. 11
         WHERE carrier_id    = @i_carrier_id
...
*      seats_free = flight_raw-seats_max - flight_raw-seats_occupied.  "Delete for Ex 11
      seats_free = flight_raw-seats_free.                               "Ex. 11
...
*                                  - connection_details-departure_time.
  ...
  METHOD get_description.
    ...
    "Ex. 09. Note any change here should also be updated in the classrun report
    DATA txt TYPE string.
    txt = 'Flight &carrierid& &connid& on  &date& from &from& to &to& '(005).
    txt = replace( val = txt sub = '&carrierid&' with = carrier_id ).
...
CLASS lcl_cargo_flight DEFINITION .
  ...
  PUBLIC SECTION.
    ...
    DATA carrier_id    TYPE /dmo/carrier_id READ-ONLY. "end of tracing carier_id type
    DATA connection_id TYPE /dmo/connection_id READ-ONLY. "end of tracing connection_id type
    DATA flight_date   TYPE /dmo/flight_date READ-ONLY.
...
CLASS lcl_cargo_flight IMPLEMENTATION.
  ...
  METHOD get_description. "continuation of tracing get_description types...

    "check carier_id to make sure it has the right type
    APPEND |Flight { carrier_id } { connection_id } on { flight_date DATE = USER } | &&
           |from { connection_details-airport_from_id } to { connection_details-airport_to_id } |
           TO r_result.
    APPEND |Planetype:     { planetype } |                         TO r_result.
...
CLASS lcl_carrier DEFINITION .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_output TYPE string.  "Ex. 06: 19.03.2026. End out-put 1
    ...
    "Ex. 6 continuation: tracing tt_output type definition
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD get_output.
    ...
    " continuation of Ex. 6: tracing the type convention under r_resuult / Ex 08.
